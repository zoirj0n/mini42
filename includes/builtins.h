...
/*                                                +#+#+#+#+#+   +#+           */
/*   Created: 2025/10/03 18:03:56 by zoisobir          #+#    #+#             */
/*   Updated: 2025/10/03 18:04:21 by zoisobir         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
void					find_and_update_oldpwd(t_shell *shell, char **env,
							char *oldpwd);
void					find_and_update_pwd(char **env, t_shell *shell);
void					move_to_home(t_shell *shell, t_exec_step *step,
							char **env, char *home);
...
void					run_export_with_args(char **args, t_shell *shell,
							bool *error);

/* ---- UNSET FUNCTIONS ---- */
void					ft_unset(t_shell *shell, t_exec_step *step);
void					unset_var(t_shell *shell, const char *var);
void					remove_env_var(t_shell *shell, char **env_copy,
							char *to_look);
void					unset_declared_var(t_shell *shell, const char *var);
void					remove_declared_env_var(t_shell *shell, char **env_copy,
							char *to_look);
